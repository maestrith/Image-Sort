...
Actions(x){
	...
	if(x~="Move_(All|Selected)_To_Current_Directory"){
		...
		if(current:=ssn(list,"*[@current]/@dir").text){
			...
			cxml.Highlight(),cxml.NoSelections()
		}else
	...
	__New(win){
		...
		for a,b in x{
			...
			if(info.4||info.5){
				...
				control:=this.xml.under(tab,"control",{hwnd:hwnd+0,track:info.4}),control.SetAttribute("type",info.1)
			}if(info.1="StatusBar")
...
class Speech{
	__New(){
		Speech.voice:=ComObjCreate("SAPI.SpInprocRecognizer"),AudioInputs:=Speech.voice.GetAudioInputs()
		if(!AudioInputs.count)
			return
		Speech.voice.AudioInput:=AudioInputs.Item[0],Speech.AudioInputs:=AudioInputs.Count(),ObjRelease(AudioInputs),Speech.Context:=Speech.voice.CreateRecoContext(),Speech.grammar:=Speech.Context.CreateGrammar(),Speech.Rules:=Speech.grammar.Rules(),ComObjConnect(Speech.Context,Speech),Speech.Rule:=Speech.Rules.Add("Move",0x1|0x20),Speech.null:=ComObjEnwrap(0),Speech.enabled:=Round(settings.ssn("//Options/@Voice").text),Speech.AddRule("GlobalSpeech",["Start Listening","Stop Listening","Switch To Global","Switch To Local","Move","Close Program","OK"],1),Speech.list:=[],list:=[]
		all:=settings.SN("//Keyboard/Global/descendant::*/@*")
		while(aa:=all.item[A_Index-1])
			Speech.list[aa.NodeName]:=aa,Speech.GlobalText.="`n" aa.NodeName
		Speech.AddRule("Global",Speech.list),Speech.AddRule("Exit",["Exit","Help","Commands",""],1)
		Loop,5
		{
			list:=[],all:=settings.SN("//Keyboard/Screen" A_Index "/descendant::*/@*")
			while(aa:=all.item[A_Index-1])
				list.push(aa.NodeName),Speech.list[aa.NodeName]:=aa
			Speech.AddRule("Screen" A_Index,list)
		}
		Speech.AddRule("SwitchTo",["Global","Local"],1)
		Speech.AddRule("GlobalNavigation",["First_Image","Back","Forward","Next"],1)
		Speech.Numbers()
	}Numbers(){
		static Modes:=[]
		for a,b in Modes
	...
	}AddRule(Name,list,State:=0,Weight:=1.0){ ;33=SRATopLevel|SRADynamic
		rule:=Speech.Rules.Add(name,33),main:=rule.InitialState()
		for a,b in list
			main.AddWordTransition(Speech.null,IsObject(b)?a:b)
		Speech.rules.Commit()
		Speech.grammar.CmdSetRuleState(name,1)
	}MoveList(info){
		...
		}Speech.Rules.Commit(),Speech.grammar.DictationSetState(0),Speech.grammar.CmdSetRuleState("Move",Speech.Enabled)
	}Toggle(x){
		for a,b in x
			Speech.Grammar.CmdSetRuleState(a,b)
		...
		if(text="Exit"){
			return t(),Speech.Commands:=0,Speech.select:=0,Speech.Toggle({Move:1,Global:1,GlobalSpeech:1}),SetTitle()
		}
		if(rule~="Part"){
		...
		if(text="Commands"){
			return Speech.grammar.CmdSetRuleState("Global",1),Speech.Commands:=1,SetTitle(),Speech.Toggle({Move:0,Global:1,GlobalSpeech:1}),t(Speech.GlobalText)
		}
		if(Speech.commands&&text="Select")
			return cxml.Select()
		if(text="select")
			return Speech.select:=1,Speech.Toggle({Move:0,Global:0,GlobalSpeech:0}),SetTitle()
		if(Speech.select)
			return
		...
		if(text="Move"){
			m("Move :)","time:.5")
			/*
				static commands that just need to be turned off and on
				commands{
					each tab has its own base list provided by the xml
					add all of the commands for each tab
					then just toggle them as you hit a tab.
					rather than rebuilding the list every time.
					have tab() report to speech
					create an object for each tab as well and have it active when that tab is active
					
				}
			*/
			/*
				Select{
					have the voice command select bring up a list of "Say 1 - " wb.images.length
					add done to it so that the user can be done selecting
				}
			*/
			/*
				This needs built up so I can say as many numbers as quickly as possible.
				give it a new rule state
				see about having different rules for different pages... that way it won't have to keep redoing the grammar
				but then again....who knows.
				have a global rule
				have a rule for commands (those won't change)
				then have rules for the move directories.
				so they can change at will.
				clear em and start over...
			*/
			return
		}
		/*
			...
			if(!Speech.Enabled)
				return ;t("You said " text " :)","time:1")
			Commands
			if(node:=list[text]){
				if(node="select"){
					return cxml.SetCurrent(cxml.xml.ssn("//item[" text "]")),cxml.Highlight()
				}
				if(node=1)
					return list:=[],t(),MoveList.Recognize()
				function:=node.NodeName
				if(IsFunc(function))
					%function%(text)
				else if(IsFunc(text))
					%text%()
				if(text="Previous_Screen"||text="Next_Screen")
					text:="Commands"
				else
					return
			}
		...
		*/
		/*
			if(text~="i)Go To"){
				m("use this for switching tabs")
				if(current:=settings.ssn("//global/dir[@name='" RegExReplace(text,"i)Go To ") "']/@dir").text){
					if(tv:=ssn(maintree.xml.find("//*/@dir",current),"@tv").text)
						Default("SysTreeView321"),TV_Modify(tv,"Select Vis Focus")
					else
						maintree.check(settings.ssn("//global/dir[@name='" RegExReplace(text,"i)Go To ") "']/@dir").text),tv:=ssn(maintree.xml.find("//*/@dir",current),"@tv").text,Default("SysTreeView321"),TV_Modify(tv,"Select Vis Focus")
				}
			}
		*/
	}
...
Compare(info:=0){
	if(info="selected"){
		...
		if(!DupCompare.SN("//compare").length)
			return
		/*
			return cxml.ClearList(),cxml.Populate(DirList.ssn("//list/item['" compare.ssn("//number/@number").text "']").text,1),Display(),cxml.Highlight()
		*/
		cxml.ClearSel(),top:=cxml.ClearList() ;,list:=compare.ssn("//compare/list"),two:=sn(list,"item")
		ea:=xml.EA(DupCompare.SSN("//compare"))
		for a,b in ea
			cxml.xml.Under(top,"item",{file:b})
		/*
			add a tag to the xml to show select to delete or rename
		*/
		Display(),cxml.SetCurrent(),cxml.Highlight()
		/*
			images:=wb.images
			while,ii:=images.item[A_Index-1].ParentNode{
				ii.GetElementsByTagName("SPAN").item[1].innerhtml:=(A_Index=1?"Select this to delete":"Select this to rename")
			}
		*/
	}
}
...
Defaults(){
	Global:={Screen:["Next_Screen","Previous_Screen"],Movement:["Left","Right","Up","Down"],Delete:["Delete"],Alt:["Alt"],XBox:["Activate_XBox_Controller"]}
	Screen1:={Actions:["Clear_All_Actions","Clear_All_Selections","Clear_Selected_Actions","Compare","Cycle_Mode","Execute_Actions","Global_Move_Mode","Local_Move_Mode","Move_All_To_Current_Directory","Move_Selected_To_Current_Directory","Move_Mode_Off","Properties","Undo","Move_Selected","Quick_Jump","Toggle_Voice_Commands","Top_Level","Auto_Action","Fix_Image"]
		    ,Navigation:["Back","Forward","First_Image"]
		    ,Selection:["Next","Select","Select_All","Toggle_FullScreen"]}
	Screen2:={Navigation:["Previous_Control","Next_Control","Refresh_Directory_List"]}
	Screen3:={ReOrder:["Move_Directory_Up","Move_Directory_Down"],Navigation:["Previous_Control","Next_Control"]}
	Screen4:={Navigation:["Previous_Control","Next_Control"]}
	...
	HotkeyObj:={Global:global,Move:Move,Screen1:Screen1,Screen2:Screen2,Screen3:Screen3,Screen4:Screen4}
...
DelRen(){
	...
	for a,b in [") To Delete This Duplicate",") To Rename It"]
		(item:=divs.item[A_Index].GetElementsByTagName("p").item[0]).innerhtml:="Select This (Press " settings.ssn("//Keyboard/Page1/@Select").text " or " settings.ssn("//Controller/Page1/@Select").text " On Your Controller" b,item.style.color:="red",item.style.fontsize:="30px"
}
Display(){
...
EditControllerAction(key){
	...
	if(v.EditingControllerInput!=0){
		...
		SplashTextOn,320,100,Press any input,% "Press any input to set the " bindings[v.EditingControllerInput] " for " main:=ctrl.GetText("HotkeysLV") "`n`nPress Escape to Exit"
	}
}
...
Gui(){
	...
	MainWin.Add("ActiveX,x0 y0 w800 h523 vWB,mshtml:,wh,1,IE","TreeView,x400 y0 w400 h450,,wh,2,DirTree","Button,xm y450,My Button,y,2","ListView,x0 y0 w400 h450 glasthotkey AltSubmit,Name|Hotkey|Full Path,h,3,Global","ListView,x400 y0 w400 h450 glasthotkey AltSubmit,Name|Hotkey|Full Path,wh,3,Local","Button,xm y450 gAddGlobal,Add &Global Directory,y,3","Button,x+M gEditGlobalHotkey,Edit Global &Hotkey,y,3","Button,x+M gAddLocal,Add &Local Directory,y,3","Button,x+M gQuickNewLocal,&Quick New Local,y,3","Button,x+M gAddAllSubs,Add All Local &Sub-Directories,y,3","Button,x+M gEditLocalHotkey,Edit Local H&otkey,y,3"
			,"TreeView,x0 y0 w200 h450 gPopKeys,,h,4,HotkeysTV","ListView,x200 y0 w600 h450 -Multi gHotkeyLV AltSubmit,Action|Keyboard|Controller Main Binding (Press A to Change)|Controller Alt Binding (Press B to Change),wh,4,HotkeysLV","Button,xm y450 gEditKeyboardHotkey Default,Edit &Keyboard Hotkey,y,4"
...
Class MoveList{
	...
	Current(dir){
		...
		add:=v.mode=4?7:8,RegExMatch(direction,"Cycle_Move_List_(.*)",found),this.Clear(),length:=this.xml.sn("//dir").length,this.next:=found1="Forward"?(this.next+add>length?0:this.next+add):(this.next-add>=0?this.next-add:(Floor(length/add)*add)),this.SetStatus()
	}Clear(){
		Loop,8
...
PopulateGlobal(){
	Default("SysListView321",1),LV_Delete(),all:=settings.sn("//global/dir"),LV_Modify(0,"-Select"),last:=settings.ssn("//settings/global/@last").text
	while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa)
		dir:=StrSplit(Trim(ea.dir,"\"),"\").pop(),ctrl.Add("global",(last=dir?"Select Vis Focus":""),dir,Convert_Hotkey(ea.Hotkey),ea.dir)
...
PopulateLocal(){
	...
	while(aa:=list.item[A_Index-1]),ea:=xml.ea(aa)
		ListView[dir:=StrSplit(Trim(ea.dir,"\"),"\").pop()]:=ctrl.Add("Local",(localdir=dir?"Select Vis Focus":""),dir,Convert_Hotkey(ea.Hotkey),ea.dir)
...
Control(move){
	...
	ControlFocus,,% "ahk_id" ssn(select,"@hwnd").text
}
Properties(value:=""){
	KeyWait,Alt,U
...
SetTitle(){
	...
	voice:=Speech.Commands?"Voice Commands: Say 'Exit' when finished":Speech.select?"Voice Select: Say 1 - " wb.images.length " : Say 'Exit' When Finished":Speech.Enabled?"Voice Enabled: Say (Help,Commands,or Select)":""
...
Help(help:="Toggle"){
	...
	v.help:=help="toggle"?(v.help:=v.help?0:1):1
	if(v.help){
		...
		if(!v.helptab){
			...
			v.helptab:=help="left"?(v.helptab-1>0?v.helptab-1:header.MaxIndex()):(v.helptab+1<=header.MaxIndex()?v.helptab+1:1)
		html:="<div style='overflow:auto;'>"
		html.="<div style='margin:10 40px !important;background-color=#ffffff;text-align:center;'>"
		html.="<header><h1>Help: " header[v.helptab] "</h1><h5>Press F1 to Open/Close this Help Screen</h5></header></div>"
		html.="<div style='margin:10 40px !important;background-color:#ffffff'>"
		html.="<p style='background-color:#ffffff'><a id='Left' style='float:left;cursor:hand;color:#0000ff;background-color:#ffffff;'><----" header[v.helptab-1<1?header.MaxIndex():v.helptab-1] "</a>"
		html.="<a id='Right' style='float:right;cursor:hand;color:#0000ff;background-color:#ffffff;'>" header[v.helptab+1>header.MaxIndex()?1:v.helptab+1] "----></a></p></br></div></br></br>"
		html.="<div style='margin:10 40px !important;background-color=#ffffff;text-align:center;margin-bottom:10px;margin-top:10px;'>"
		for a,b in info[v.helptab]
			html.="<p>" b "<p>"
		html.="</div>"
		html.="</div></div>",wb.body.innerhtml:=html,wb.GetElementsByTagName("div").item[0].style.height:=mainwin.WinPos().h
		...
		rem:=wb.GetElementsByTagName("div").item[0],rem.ParentNode.RemoveChild(rem)
		div:=wb.CreateElement("div")
		wb.write("<!DOCTYPE html><html><body leftmargin=0 topmargin=0 rightmargin=0 bottommargin=0></body><div></div></html>"),b.body.style.backgroundcolor:=0
		Display(),cxml.Highlight()
		if(v.helptab=1)
			GuiControl,1:+Redraw,AtlAxWin1
		v.helptab:=0
